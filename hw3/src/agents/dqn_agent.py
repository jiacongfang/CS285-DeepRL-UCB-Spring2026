...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        with torch.no_grad():
            qa_values = self.critic(observation)  # (1, num_actions)
            greedy_action = torch.argmax(qa_values, dim=1)  # shape: (1,)

        if np.random.rand() < epsilon:
            action = torch.randint(
                low=0,
                high=self.num_actions,
                size=greedy_action.shape,
                device=greedy_action.device,
            )
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qa_target_values = self.target_critic(next_obs)

            if self.use_double_q:
                # TODO(Section 2.5): implement double-Q target action selection
                next_action = torch.argmax(next_qa_values, dim=1)
            else:
                next_action = torch.argmax(next_qa_target_values, dim=1)

            next_q_values = next_qa_target_values.gather(1, next_action.unsqueeze(1)).squeeze(1)   
